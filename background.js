...
async function analyzeTextOnly(text, baseData) {
    ...
    const systemPrompt = `شما متخصص استخراج اطلاعات ساختاریافته از نامه‌های اداری رسمی ایران هستید.

فرمت خروجی - فقط JSON معتبر (بدون هیچ متنی قبل یا بعد):
{
  "originNo": "شماره دقیق نامه از هدر سند",
  "originDate": { "day": "DD", "month": "MM", "year": "YY" },
  ${isFully ? '' : '"sender": "نام سازمان فرستنده",\n  "senderEmail": "ایمیل فرستنده",'}
  "subject": "موضوع تصحیح‌شده (بدون شماره نامه)",
  "description": "متن کامل نامه بازنویسی‌شده بدون تکرار و حاشیه",
  "keywords": "نام اشخاص، کدملی، نام سازمان‌ها جدا با ویرگول"
}

قوانین حیاتی:
1. شماره نامه (originNo) و تاریخ (originDate) را فقط از <PRIMARY_DOCUMENT_HEADER> بردار - هرگز از متن پیوست یا عطف نامه
2. originNo هرگز نباید تاریخ باشد. نامه‌های اداری: فرمت سال/سریال/کد (مثل 1405/18384/44/56/ص). نامه‌های قضایی: عدد ۱۶-۱۸ رقمی بدون اسلش
3. originDate: day و month دو رقمی انگلیسی (01-31 و 01-12). year فقط دو رقم آخر سال شمسی (مثلاً 1405 → "05")
4. تاریخ نامه هرگز نباید در آینده باشد (بزرگتر از ${todayDateStr}). اگر تاریخ OCR نامعتبر بود، تصحیح کن
5. سال‌های شمسی اداری جاری: 1403، 1404 یا 1405. سال سه‌رقمی (مثل 405) → 1405
6. خطای فونت نستعلیق: ماه 02 یا 09 در شهریور (ماه 06) → باید 06 باشد`;
